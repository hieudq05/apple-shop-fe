...
import { ChevronRightIcon, ExclamationCircleIcon, CheckCircleIcon } from "@heroicons/react/24/outline";
import axios from "axios";
import { useNavigate, useLocation } from "react-router-dom";
import { useAuth } from "../contexts/AuthContext";
import { setAccessToken, setRefreshToken } from "../utils/storage";
import type { OtpValidationRequest, ApiResponse, AuthenticationResponse } from '../types/api';
...
const OtpVerificationPage: React.FC = () => {
    ...
    const handleVerifyOtp = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (error.response?.data?.error?.errors) {
                ...
                setErrors([error.response?.data?.error?.errorMessage || error.response?.data?.message || 'Xác thực OTP thất bại']);
            }
        } finally {
            setIsLoading(false);
        }
    ...
    const handleResendOtp = async () => {
        ...
        try {
            const response = await axios.post("http://localhost:8080/api/v1/otp/send", {
                email: email
            });
            ...
            setErrors([error.response?.data?.message || 'Gửi lại mã OTP thất bại']);
        } finally {
                    ...
                    <div className={"flex-1 size-full flex flex-col space-y-12"}>
                        ...
                        <div className="flex flex-col items-center space-y-4">
                            {timeLeft > 0 ? (
                                <div className="text-gray-600 text-sm">
                                    Mã sẽ hết hạn sau: <span className="font-mono font-semibold">{formatTime(timeLeft)}</span>
                                </div>
                            ) : (
                        ...
                        <div className={"flex flex-col space-y-2 items-center"}>
                            <div className={"flex items-center gap-1"}>
                                <span className={"text-gray-500 text-sm"}>Bạn muốn thay đổi email? </span>
                                <button
