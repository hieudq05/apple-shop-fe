...
import { ProductDataTable, type Product } from '@/components/product-data-table';
...
const transformToProduct = (adminProduct: AdminProduct): Product => {
    return {
        ...
        stocks: adminProduct.stocks?.map(stock => ({
            id: stock.id,
            ...
            price: stock.price,
            productPhotos: stock.photos?.map((photo, index) => ({
                id: index,
                imageUrl: photo,
                alt: `${adminProduct.name} - ${stock.color.name}`
            })) || []
        })) || [],
        features: adminProduct.features?.map(feature => ({
            id: feature.id,
            name: feature.name,
...
const AdminProductsPage: React.FC = () => {
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Products Table */}
            {!isLoading && !isErrorProducts && productsData?.products && productsData.products.length > 0 && (
<div>
                    <CardContent className="pb-4 px-0">
                        <ProductDataTable 
                            data={productsData.products}
                            // Navigate to product view by react-router-dom
                            onView={(productId, categoryId) => window.open(`/admin/products/${categoryId}/${productId}`, '_blank')}
                            onEdit={(productId, categoryId) => window.open(`/admin/products/${categoryId}/${productId}/edit`, '_blank')}
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center space-x-2">
                            ...
                            <div className="flex items-center space-x-1">
                                {[...Array(Math.min(metadata.totalPage, 5))].map((_, i) => {
                                    const pageNumber = metadata.currentPage > 2 ? metadata.currentPage - 2 + i : i;
                                    return pageNumber < metadata.totalPage ? (
                                        <Button
                                            key={pageNumber}
                                            className={`bg-transparent hover:bg-transparent text-black shadow-none ${metadata.currentPage === pageNumber ? "underline" : ""}`}
                                            size="sm"
                                            onClick={() => setMetadata(prev => ({...prev, currentPage: pageNumber}))}
                                        >
                                            {pageNumber + 1}
                                        </Button>
                                    ) : null;
