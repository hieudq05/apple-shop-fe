...
interface ProductCartProps extends CartItem {
    onQuantityChange?: (itemId: string, colorId: string, storageId: string, newQuantity: number) => void;
    onRemoveItem?: (itemId: string, colorId: string, storageId: string) => void;
}
...
const ProductCart: React.FC<ProductCartProps> = ({
    ...
    const handleQuantityChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        if (onQuantityChange) {
            onQuantityChange(id, color.id, storage.id, newQuantity);
        }
    };
    ...
    return (
        <div className={"border-t py-20 grid-cols-6 grid gap-12"}>
            <div
                className={"col-span-2 xl:col-span-1 overflow-hidden w-full rounded-2xl aspect-[8/10] flex justify-center items-center"}>
                <img src={imageUrl} alt={name}
                     className={"w-full h-full object-cover"}/>
            </div>
            <div className={"md:grid md:grid-cols-5 w-full gap-4 col-span-4 xl:col-span-5 flex flex-col"}>
                <div className={"col-span-3 space-y-4"}>
                    <Link to={`/product/${id}`}
                          className={"text-xl font-semibold hover:text-blue-600 transition"}>{name} {storage.name} {color.name}</Link>
                    <div className={"flex gap-4"}>
                        <svg
                            className="as-svgicon-rtl-mirrored as-svgicon as-svgicon-boxtruck as-svgicon-tiny as-svgicon-boxtrucktiny"
                            viewBox="0 0 21 21" role="img" aria-hidden="true" width="28px"
                            height="28px">
                            <path fill="none" d="M0 0h21v21H0z"></path>
                            <path fill="#1d1d1f"
                                  d="M19.559 10.274 17.24 7.53A1.688 1.688 0 0 0 15.918 7H15v-.75A2.25 2.25 0 0 0 12.75 4h-8.5A2.25 2.25 0 0 0 2 6.25V13a2.25 2.25 0 0 0 2.25 2.25h.56A2.248 2.248 0 0 0 7 17a2.202 2.202 0 0 0 2.19-2H14a2.214 2.214 0 0 0 2.25 2 2.248 2.248 0 0 0 2.19-1.75h.016A1.4 1.4 0 0 0 20 13.747v-2.363a1.61 1.61 0 0 0-.441-1.11ZM8.142 15.25a1.245 1.245 0 0 1-2.284 0 1.212 1.212 0 0 1 0-1 1.245 1.245 0 0 1 2.284 0 1.212 1.212 0 0 1 0 1ZM9.15 14a2.267 2.267 0 0 0-4.34.25h-.56A1.252 1.252 0 0 1 3 13V6.25A1.251 1.251 0 0 1 4.25 5h8.5A1.251 1.251 0 0 1 14 6.25V14Zm8.242 1.25a1.245 1.245 0 0 1-2.284 0 1.212 1.212 0 0 1 0-1 1.245 1.245 0 0 1 2.284 0 1.212 1.212 0 0 1 0 1ZM19 13.747c0 .334-.084.503-.544.503h-.016A2.245 2.245 0 0 0 15 12.88V8h.918a.681.681 0 0 1 .592.211l2.324 2.752a.617.617 0 0 1 .166.42Z"></path>
                        </svg>
                        <div className={"text-sm"}>
                            <p>Đặt hàng trong hôm nay. Giao hàng
                                đến: $$$</p>
                            <p className={"font-semibold"}>{new Date().toLocaleString('vi-VN',
                                {
                                    year: 'numeric',
                                    month: '2-digit',
                                    day: '2-digit',
                ...
                <div className={"flex-1"}>
                    <select
                        className="appearance-none rounded-lg ps-2 bg-white text-2xl font-medium focus:outline-none w-14 cursor-pointer"
                        value={quantity}
                ...
                <div className={"flex flex-col items-end md:text-right text-left"}>
                    ...
                    {onRemoveItem && (
                        <div className={"w-full flex md:justify-end justify-start md:mt-4 md:mb-0 mb-4"}>
                            <button
                                ...
                                onClick={() => onRemoveItem(id, color.id, storage.id)}
                            >
                                Xóa
                            </button>
                        </div>
